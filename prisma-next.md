...
Commit both files to git. When you change your contract, run `yarn prisma-next contract emit` to update them.
...
```bash
yarn prisma-next contract emit       # Update contract.json and contract.d.ts
yarn prisma-next db init             # Create tables in the database
yarn prisma-next migration status    # Show migration status
```
...
2. Run `yarn prisma-next contract emit` to regenerate the contract.
